 ...
 * extension.
 */
class CRM_Sparkpost_ExtensionUtil {
  const SHORT_NAME = 'sparkpost';
  const LONG_NAME = 'sparkpost';
  const CLASS_PREFIX = 'CRM_Sparkpost';
...
spl_autoload_register('_sparkpost_civix_class_loader', TRUE, TRUE);

function _sparkpost_civix_class_loader($class) {
  if ($class === 'CRM_Sparkpost_DAO_Base') {
    if (version_compare(CRM_Utils_System::version(), '5.74.beta', '>=')) {
      class_alias('CRM_Core_DAO_Base', 'CRM_Sparkpost_DAO_Base');
      // ^^ Materialize concrete names -- encourage IDE's to pick up on this association.
    }
    else {
      $realClass = 'CiviMix\\Schema\\Sparkpost\\DAO';
  ...
  if (strpos($class, 'CiviMix\\Schema\\Sparkpost\\') === 0) {
 ...
 */
function _sparkpost_civix_civicrm_enable(): void {
  _sparkpost_civix_civicrm_config();
  // Based on <compatibility>, this does not currently require mixin/polyfill.php.
}
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _sparkpost_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item);
      }
    }
    return $found;
  }
